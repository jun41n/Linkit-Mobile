  ...
  return (
    <View style={styles.moodBarRow}>
      ...
      <View style={[styles.moodBarTrack, { backgroundColor: colors.muted }]}>
        <View style={[styles.moodBarFill, { backgroundColor: mood.color, width: `${Math.round(pct * 100)}%` }]} />
      </View>
...
export default function StatsScreen() {
  ...
  const stats = useMemo(() => {
    ...
    const daily = records.filter((r) => r.type === "DAILY").length;
    const share = records.filter((r) => r.type === "SHARE").length;
    ...
    const avgWords = records.filter((r) => r.content).length > 0 ? Math.round(records.reduce((s, r) => s + (r.content?.split(/\s+/).length || 0), 0) / records.filter((r) => r.content).length) : 0;
    ...
    return { total, travel, daily, share, withPhoto, withMood, totalExpense, avgWords, moodEntries, topMood, withMoodCount: records.filter((r) => r.moodTag).length };
  }, [records]);
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}> 
      ...
      <ScrollView contentContainerStyle={[styles.scrollContent, { paddingBottom: bottomPad + 100 }]} showsVerticalScrollIndicator={false}>
        <View style={styles.bigStatsRow}>
          <LinearGradient colors={[colors.gradientStart, colors.gradientEnd]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.bigStat}>
          ...
          <View style={styles.smallStats}>
            ...
            <View style={[styles.smallStat, { backgroundColor: colors.card, borderColor: colors.border }]}>
              ...
              <Text style={[styles.smallStatValue, { color: colors.foreground }]}>{stats.share}</Text>
              <Text style={[styles.smallStatLabel, { color: colors.mutedForeground }]}>교환일기</Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.cardRow}>
          <View style={[styles.infoCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
            <Ionicons name="airplane-outline" size={20} color={colors.travelActive} />
            <Text style={[styles.infoValue, { color: colors.foreground }]}>{stats.travel}</Text>
            <Text style={[styles.infoLabel, { color: colors.mutedForeground }]}>여행</Text>
          </View>
          <View style={[styles.infoCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
            <Ionicons name="images-outline" size={20} color={colors.accent} />
            <Text style={[styles.infoValue, { color: colors.foreground }]}>{stats.withPhoto}</Text>
            <Text style={[styles.infoLabel, { color: colors.mutedForeground }]}>사진</Text>
          </View>
          <View style={[styles.infoCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
            <Ionicons name="wallet-outline" size={20} color={colors.primary} />
            <Text style={[styles.infoValue, { color: colors.foreground }]} numberOfLines={1}>{stats.totalExpense > 0 ? stats.totalExpense.toLocaleString() : "-"}</Text>
            <Text style={[styles.infoLabel, { color: colors.mutedForeground }]}>지출</Text>
          </View>
        </View>
        ...
        {records.length === 0 && (
          <View style={styles.empty}>
            ...
            <Text style={[styles.emptyDesc, { color: colors.mutedForeground }]}>기록을 남기면 여기서 분석 결과를 볼 수 있습니다</Text>
          </View>
        )}
      </ScrollView>
    </View>
...
const styles = StyleSheet.create({
  ...
  bigStat: { flex: 1.2, padding: 20, borderRadius: 20, alignItems: "center", justifyContent: "center", gap: 4 },
  bigStatValue: { fontSize: 42, fontFamily: "Inter_700Bold", color: "#fff" },
  bigStatLabel: { fontSize: 13, fontFamily: "Inter_500Medium", color: "rgba(255,255,255,0.8)" },
  smallStats: { flex: 1, gap: 12 },
  smallStat: { flex: 1, alignItems: "center", justifyContent: "center", borderRadius: 16, borderWidth: 1, gap: 2 },
  smallStatValue: { fontSize: 22, fontFamily: "Inter_700Bold" },
  smallStatLabel: { fontSize: 11, fontFamily: "Inter_400Regular" },
  cardRow: { flexDirection: "row", gap: 10 },
  infoCard: { flex: 1, padding: 14, borderRadius: 16, borderWidth: 1, alignItems: "center", gap: 4 },
