...
- **⚠️ TODO before launch — RE-ENABLE AUTH GATE**: For dev convenience the auth gate in `app/(tabs)/_layout.tsx` was removed and a "로그인 없이 둘러보기" guest link was added in `app/(auth)/sign-in.tsx`. Before final launch: (1) restore the original tabs gate (`useAuth` + `if (!isLoaded) return null; if (!isSignedIn) return <Redirect href="/(auth)/sign-in" />;`) and re-add the `import { useAuth } from "@clerk/expo"` and `Redirect` to expo-router import in that file; (2) remove the guest `<Link href="/(tabs)">로그인 없이 둘러보기 →</Link>` block and its `guestRow`/`guestText` styles from sign-in.tsx. Reference commit before removal: `877cb38` (gate intact). Reference commit after removal: `35337e5`.
